...
def create_resource(module, client, token):
    """Create the resource."""
    body = build_body("GatewayCreateProducerOracleDb", dict(module.params, token=token))
    return call_api(module, client, "gateway_create_producer_oracle_db", body)
...
def update_resource(module, client, token):
    ...
    body = build_body("GatewayUpdateProducerOracleDb", dict(module.params, token=token))
    return call_api(module, client, "gateway_update_producer_oracle_db", body)
...
def main():
    ...
    if state == 'absent':
        ...
        module.exit_json(changed=False, msg="gateway_producer_oracle already absent")
    else:
        if current is None:
