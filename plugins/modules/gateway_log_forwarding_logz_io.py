...
def create_resource(module, client, token):
    """Create the resource."""
    body = build_body("GatewayUpdateLogForwardingLogzIo", dict(module.params, token=token))
    return call_api(module, client, "gateway_update_log_forwarding_logz_io", body)
...
def main():
    ...
    if state == 'absent':
        ...
        module.exit_json(changed=False, msg="gateway_log_forwarding_logz_io already absent")
    else:
        if current is None:
